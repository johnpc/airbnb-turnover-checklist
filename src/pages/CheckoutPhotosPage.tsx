...
import { useCreateCheckoutPhoto, useCheckoutPhotos } from '@/hooks/use-photos'
...
export function CheckoutPhotosPage() {
  ...
  const { data: currentStayPhotos } = useCheckoutPhotos(stayId!)
  ...
  const capturePhoto = async () => {
    ...
    try {
      ...
      if (existingPhoto) {
        await client.models.CheckoutPhoto.delete({ id: existingPhoto.id })
      }
  ...
  return (
    <div className="container mx-auto p-4 max-w-2xl space-y-4">
      <CapturePhotoCard
        ref={captureCardRef}
        listingName={`Checkout Photos - ${listing.name}`}
      ...
      <StayPhotosCard photos={currentStayPhotos} photoType="checkout" onRetake={handleRetake} />
    </div>
  )
}
